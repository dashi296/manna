...
- [ ] **Step 3: リファクタ後も既存テストが green であることを確認する**
...
- [ ] **Step 3: `scriptures.json` の `bofm.books` 配列の先頭に5書を追加する**
...
- [ ] **Step 4: `getScriptureLabel` / `buildScriptureUrl` を front matter 対応にする**
...
- [ ] **Step 1: `export-books-seed.mjs` を UPSERT 対応に書き換える**
...
- [ ] **Step 1: `parseParagraphs` の import を追加し `buildChapterList()` に `isFrontMatter` を含める**
...
- [ ] **Step 2: `fetchChapter()` に front matter 用の URI 分岐を追加する**
...
- [ ] **Step 4: 変更後のファイル全体を読み、分岐漏れがないか確認する**
...
- [ ] **Step 3: `VerseView` の `backLabel` を front matter 対応にする**
...
- [ ] **Step 4: `ChapterView` の `title` を front matter 対応にする**
...
- [ ] **Step 3: 章数表示を front matter で条件分岐する**
...
- [ ] **Step 3: `buildChapterItems` を `model.ts` に実装する**
...
- [ ] **Step 4: `index.ts` で公開APIとしてエクスポートする**
...
- [ ] **Step 6: `ScriptureSelector.tsx` で `buildChapterItems` を使うようにする**
...
- [ ] **Step 7: pwa の全テストを実行し回帰がないことを確認する**
...
- [ ] **Step 5: 前付け文書のデータが揃っていることをDBで検証する**
...
- [ ] **Step 6: フロントエンドが実データで正しく表示されることを確認する**
