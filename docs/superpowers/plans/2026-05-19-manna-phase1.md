...
├── app/
│   ├── routes/
│   │   ├── __root.tsx              # ルートレイアウト（BottomNav含む）
│   │   ├── index.tsx               # / フィード
│   │   ├── login.tsx               # /login
│   │   ├── scriptures/
│   │   │   ├── index.tsx           # /scriptures（聖典集一覧）
│   │   │   └── $collection/
│   │   │       ├── index.tsx       # /scriptures/$collection（書籍一覧）
│   │   │       └── $book/
│   │   │           ├── index.tsx   # /scriptures/$collection/$book（章一覧）
│   │   │           └── $chapter.tsx # /scriptures/$collection/$book/$chapter（節一覧）
│   │   ├── posts/
│   │   │   ├── new.tsx             # /posts/new（投稿作成）
│   │   │   └── $id.tsx             # /posts/$id（投稿詳細）
│   │   ├── profile/
│   │   │   ├── index.tsx           # /profile（自分）
│   │   │   └── $userId.tsx         # /profile/$userId（他ユーザー）
│   │   └── notifications.tsx       # /notifications
│   ├── components/
│   │   ├── AppLayout.tsx           # 認証ガード + レイアウトラッパー
│   │   ├── BottomNav.tsx           # 下部ナビゲーション
│   │   ├── PostCard.tsx            # フィードのカード
│   │   ├── PostEditor.tsx          # Markdownエディタ + 投稿フォーム全体
...
│   │   └── Avatar.tsx              # アバター画像
│   ├── lib/
│   │   ├── supabase.ts             # Supabaseクライアント（browser/server両対応）
│   │   ├── scripture.ts            # 公式URLビルダー + 書誌データヘルパー
│   │   └── auth.ts                 # セッション取得ヘルパー
│   ├── data/
│   │   └── scriptures.json         # 静的書誌データ（書籍・章数）
│   └── types/
│       └── database.ts             # Supabase CLIで生成した型
├── supabase/
│   ├── migrations/
...
Expected: `app/types/database.ts` が生成される。
...
**Files:**
- Create: `app/data/scriptures.json`
- Create: `app/lib/scripture.ts`
- Create: `tests/lib/scripture.test.ts`
...
import { buildScriptureUrl, getScriptureLabel, getBook, getCollection } from '../../app/lib/scripture'
...
```bash
npx vitest run tests/lib/scripture.test.ts
```
...
import { getAllCollections } from '../../lib/scripture'
...
import { getCollection } from '../../../lib/scripture'
...
```typescript
import { createFileRoute, Link, notFound } from '@tanstack/react-router'
import { getBook, getCollection } from '../../../../lib/scripture'
...
```typescript
import { createFileRoute, Link, notFound } from '@tanstack/react-router'
import { getBook, buildScriptureUrl, getScriptureLabel } from '../../../../lib/scripture'
...
- Create: `app/components/ScriptureSelector.tsx`
- Create: `app/components/VisibilitySelector.tsx`
- Create: `app/components/PostEditor.tsx`
- Create: `app/routes/posts/new.tsx`
- Create: `tests/components/VisibilitySelector.test.tsx`

- [ ] **Step 1: VisibilitySelectorのテストを書く**

`tests/components/VisibilitySelector.test.tsx`:

```typescript
import { describe, it, expect, vi } from 'vitest'
import { render, screen } from '@testing-library/react'
import userEvent from '@testing-library/user-event'
import { VisibilitySelector } from '../../app/components/VisibilitySelector'
...
```bash
npx vitest run tests/components/VisibilitySelector.test.tsx
```
...
`app/components/VisibilitySelector.tsx`:
...
`app/components/ScriptureSelector.tsx`:

```typescript
import { useState } from 'react'
import { getAllCollections, getBook } from '../lib/scripture'
...
- Create: `app/components/FamilyButton.tsx`
...
`app/components/FamilyButton.tsx`:
