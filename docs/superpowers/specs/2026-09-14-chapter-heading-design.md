...
タイトルは中央寄せ、概要は左寄せにする（数行の文章を中央に置くと読みにくい）。なお**ヘッダー側の
`h1` は左寄せのまま**で、画面内で揃っていない。ヘッダーは10画面で共有しており、そこだけ直すと
画面間で不揃いになるため #157 に切り出した。

節の左右の余白は `VerseRow` 自身の `px-4` だけにする。ただし右に 4px だけ確保する。
印のバッジ（`VerseCommentGutter` の `-right-1`）がはみ出す作りで、これまではページ側の余白に
逃がしていた。無くすとページャ（`overflow-x: auto`）に切られる。**本体とプレビューの両方に
同じ余白を入れること。** 片方だけだと折り返しがずれて高さが食い違う。
