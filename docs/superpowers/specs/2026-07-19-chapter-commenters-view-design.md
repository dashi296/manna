...
**Note (2026-07-19 更新):** 当初は `?user=<UUID>` パラメータで選択ユーザを持たせていたが、URL が長くなり章跨ぎで選択が失われる問題があったため、`zustand` + `persist` middleware（localStorage キー `manna:selectedUser:v1`）で保持する形に切り替えた。URL からは撤去。

`view=count` は URL に載せない方針を継続。
...
| `?view=who&mode=select` の同時指定 | 選択モード優先（既存の `SelectionModeHeader` に置き換わる）。マーカー・吹き出しも隠す |
| `件数` に戻したときの store 選択 | `setView('count')` 実行時に store も自動 clear |
| localStorage の selectedUserId が今の章に投稿していない | 章 loader で `chapterCommenters` に含まれない → client 側で `selectedUser = null` にサイレントフォールバック |
...
さらに 2026-07-19 追加で、選択ユーザの持ち方を **URL クエリ (`?user=<id>`) から zustand + localStorage (`manna:selectedUser:v1`) に切り替え**。動機:

- URL からユーザ ID (UUID) が消えて共有 URL がクリーンになる
- 章跨ぎで選択が維持される（`/scriptures/bofm/1-ne/3` → `/1-ne/4` に移動しても選択保持）
- loader は widened `POST_SELECT` で circle 全員の投稿を返し、client 側で `selectedUserId` を使って `selectedUserPosts` / `versesWithMarker` / `postsByVerse` を導出
