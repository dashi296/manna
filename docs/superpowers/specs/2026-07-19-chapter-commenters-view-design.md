...
新規 widget `widgets/chapter-comment-rail/` — デスクトップ専用:
...
- レール内は各コメントを新規 `CompactPostCard`（後述）で並べ、`created_at` 降順
...
- Sheet 中身: その節に対する選択ユーザーの投稿を新着順で全件（複数あればスクロール）。カードは `CompactPostCard`
- Sheet ヘッダー: `📖 第N章 節M — 中村さん`
- Sheet 内フッター: `プロフィールを見る →`（プロフィール遷移）

widget を 2 つに分ける理由: 呼び出し側（`chapter.tsx`）で `useIsMobile` により表示先を選ぶことで、各 widget の責任がターゲットサイズを意識しなくてよい構造にする。

## CompactPostCard

新規 `shared/ui/CompactPostCard.tsx`:

- `PostCard` と同等の情報を持つ縮小版（アバター小、パディング詰め、`created_at` 相対表示）
- 右レール / モバイル Sheet の両方で使う
- コンテンツ本文は 3 行 clamp + 「もっと読む」で単独ページへ
- 背景色・区切り線は使う側（レール / Sheet）でオーバーライド可能（`className` 経由）

`PostCard` に `compact` variant を足す形も検討したが、右レール専用の視覚要件（clamp、単独スクロール環境）が既存カードと十分に離れているため、別コンポーネントに切り出す。
...
| `widgets/verse-comment-sheet/ui/VerseCommentSheet.tsx` | 新規 | モバイル Sheet |
| `widgets/verse-comment-sheet/index.ts` | 新規 | Public API |
| `shared/ui/CompactPostCard.tsx` | 新規 | 縮小版 PostCard（右レール / Sheet 両用） |
