...
- 文字サイズは **A（据え置き: `text-sm` / `leading-relaxed`）を採用**した。いったん B（`text-base` / `leading-[1.9]`）を適用したが、実機で確認したうえで据え置きに戻した（コミット `332977f` とその revert）。フォントの差し替えのみを変更範囲とし、レイアウトは現行のまま保つ
...
- ルビは明朝体でも潰れず、既存の `rt { transform: translateY(0.25em) }` の調整量は適切だった
